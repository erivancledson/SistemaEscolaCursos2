...
public class TelaListarCurso extends JFrame {
	...
	private void initialize() {
		...
		colunaTabelaCurso = new String[] {"ID", "NOME", "DIA", "HORARIO", "CAMINHO IMAGEM"};
	...
	private void listarTodosCursos() {
		...
		try {
			ArrayList<Curso> listar = Fachada.getInstance().cursoListar();
			for(Curso curso: listar) {
				Vector vector = new Vector();
				vector.add(curso.getCod_curso());
				vector.add(curso.getNome());
				vector.add(curso.getDia_semana());
				vector.add(curso.getHorario());
				vector.add(curso.getImagem());
